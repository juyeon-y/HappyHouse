...
export default {
  ...
  computed: {
    ...mapState(houseStore, ["houses", "house", "zoomLevel", "currCategory", "resetMapChk"]),
  },
  watch: {
    houses: function () {
      this.setMarkers();
    ...
    resetMapChk: function () {
      console.log("reset map " + this.resetMapChk);
      this.removeMarker();
      this.removePlaceMarker();
    },
  },
  methods: {
    ...
    initMap() {
      ...
      const options = {
        //지도를 생성할 때 필요한 기본 옵션
        center: new kakao.maps.LatLng(37.50128066622659, 127.03966686134562), //지도의 중심좌표.
    ...
    displayPlaceInfo(place) {
      ...
      this.placeOverlay.setPosition(new kakao.maps.LatLng(+place.y + 0.00015, +place.x - 0.0004));
...
.placeinfo .title {
  ...
  color: white;
  background: #d95050;
  background: #e82313 url(https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/arrow_white.png)
